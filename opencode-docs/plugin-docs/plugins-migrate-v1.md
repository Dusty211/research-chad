# Migrate plugins from V1 | OpenCode
...
## Migrate plugin configuration[#](#migrate-plugin-configuration)
...
## Replace the entrypoint[#](#replace-the-entrypoint)
...
## Migrate the context[#](#migrate-the-context)
...
The context exposes many more domains than the table lists, including catalogs, commands, integrations, MCP servers, references, skills, storage, VCS, worktrees, and web search. See the [complete plugin context](/v2/docs/build/plugins#api).

## Register hooks in `setup`[#](#register-hooks-in-setup)
...
## Migrate request hooks[#](#migrate-request-hooks)
...
## Migrate custom tools[#](#migrate-custom-tools)
...
## Migrate events and cleanup[#](#migrate-events-and-cleanup)
...
## Migrate options and persistent state[#](#migrate-options-and-persistent-state)
...
## Support V1 and V2 from one package[#](#support-v1-and-v2-from-one-package)
...
## Verify a ported plugin[#](#verify-a-ported-plugin)
...
See the [Plugins guide](/v2/docs/build/plugins) for the full Promise API, [Effect plugins](/v2/docs/build/plugins/effect) for the Effect entrypoint, [plugin RPC](/v2/docs/build/plugins/rpc) for custom methods and events, and [CLI plugins](/v2/docs/build/plugins/cli) for TUI extensions.
