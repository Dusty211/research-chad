...
Source: https://opencode.ai/v2/docs/references/

References give OpenCode named access to directories outside the current project. Add one in `opencode.json` or `opencode.jsonc`, then attach its alias in your client when you need it.
...
Without `branch`, OpenCode checks out and refreshes the remote’s default branch. Branch names may contain letters, numbers, `/`, `_`, `.`, and `-`, but cannot start with `-` or contain `..`.
...
A checkout is eligible when its last refresh attempt was at least 24 hours ago, or when no attempt has been recorded. A refresh fetches and resets to the configured branch or the remote’s default branch.
...
`description` tells agents when a reference is relevant. References with a description appear in agent instructions with their alias and resolved path.
...
`hidden` affects only interactive visibility. The reference remains in the reference API and, when it has a description, in agent instructions.

## Usage[#](#usage)

Clients attach a reference by its root alias. The attachment contains a non-recursive listing of the root’s immediate files and directories.
...
References do not grant extra tool permissions. Access outside the active Location still follows normal tool rules and the `external_directory` permission; editing also requires the applicable edit permission.
