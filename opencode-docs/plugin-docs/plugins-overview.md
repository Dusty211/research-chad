...
Plugins can modify OpenCode’s behavior and add new features. To change the terminal UI, build a [CLI plugin](/v2/docs/build/plugins/cli).

Migrating an existing OpenCode 1 plugin? Follow the [V1 plugin migration guide](/v2/docs/build/plugins/migrate-v1).
...
See [Configure plugins](/v2/docs/plugins) for more loading and configuration options.
...
The plugin context is essentially an [OpenCode server client](/v2/docs/build/client). Its read and action methods use the same inputs and responses as the client. It adds plugin-only methods for transforms, runtime hooks, reloads, registrations, and plugin options.
...
After provider transforms and availability are resolved, OpenCode materializes the active providers’ models and replays every model transform in order. A model policy can inspect the whole candidate collection, including newly refreshed sources.
...
Captured inputs are not watched automatically. Load external data before the synchronous callback, then call `reload()` after those inputs change. Here, `loadFromSource()` returns `{ info: Provider.Info, models: readonly Model.Info[] }` entries.
...
Provider changes invalidate the whole active model result, so the output-price policy still filters the refreshed models. Use `ctx.model.reload()` when only an input captured by a model transform changes.
...
Schema: [`Agent.Info`](/v2/docs/api#schema-Agent.Info)
...
Schemas: [`Provider.Info`](/v2/docs/api#schema-Provider.Info), [`Model.Info`](/v2/docs/api#schema-Model.Info)
...
Callbacks edit raw model overrides. Provider defaults are merged once when OpenCode commits the result; unchanged reads reuse that result. `editor.provider` exposes immutable source definitions, including inactive templates, rather than model-transform output.
...
Schema: [`Model.Info`](/v2/docs/api#schema-Model.Info)
...
Schemas: [`Command.Info`](/v2/docs/api#schema-Command.Info), [`Session.Inbox.Delivery`](/v2/docs/api#schema-Session.Inbox.Delivery)
...
Schemas: [`Integration.Info`](/v2/docs/api#schema-Integration.Info), [`Integration.Method`](/v2/docs/api#schema-Integration.Method), [`Connection.Info`](/v2/docs/api#schema-Connection.Info), [`Form.Answer`](/v2/docs/api#schema-Form.Answer)
...
Schemas: [`Mcp.Server`](/v2/docs/api#schema-Mcp.Server), [`Mcp.LocalConfigEncoded`](/v2/docs/api#schema-Mcp.LocalConfigEncoded), [`Mcp.RemoteConfigEncoded`](/v2/docs/api#schema-Mcp.RemoteConfigEncoded)
...
Schemas: [`Plugin.Info`](/v2/docs/api#schema-Plugin.Info), [`Plugin.Source`](/v2/docs/api#schema-Plugin.Source)
...
Schemas: [`Reference.Info`](/v2/docs/api#schema-Reference.Info), [`Reference.LocalSource`](/v2/docs/api#schema-Reference.LocalSource), [`Reference.GitSource`](/v2/docs/api#schema-Reference.GitSource)
...
Replace the session-scoped permission rules. They are evaluated after the agent’s rules, and the last matching rule wins. Child sessions inherit the rules in effect when they are created.
...
Schemas: [`Session.Info`](/v2/docs/api#schema-Session.Info), [`Model.Ref`](/v2/docs/api#schema-Model.Ref), [`Session.Inbox.User`](/v2/docs/api#schema-Session.Inbox.User), [`Session.Inbox.Synthetic`](/v2/docs/api#schema-Session.Inbox.Synthetic)
...
Schema: [`Skill.Info`](/v2/docs/api#schema-Skill.Info)
...
Schemas: [`Tool.Content`](/v2/docs/api#schema-Tool.Content), [`Tool.TextContent`](/v2/docs/api#schema-Tool.TextContent), [`Tool.FileContent`](/v2/docs/api#schema-Tool.FileContent)
...
Schemas: [`Vcs.Info`](/v2/docs/api#schema-Vcs.Info), [`Vcs.FileStatus`](/v2/docs/api#schema-Vcs.FileStatus), [`FileDiff.Info`](/v2/docs/api#schema-FileDiff.Info)
...
Schemas: [`WebSearch.Provider`](/v2/docs/api#schema-WebSearch.Provider), [`WebSearch.Result`](/v2/docs/api#schema-WebSearch.Result)
...
Schema: [`V2EventEncoded`](/v2/docs/api#schema-V2EventEncoded)
...
The `./rpc` export is optional; include it when publishing a shared [RPC contract](/v2/docs/build/plugins/rpc) for other plugins and clients to import without loading your implementation.

Use versions compatible with the OpenCode release you target and test the installed package, not only a workspace-linked copy. Publish a compatible plugin update when you adopt a newer API contract.

## Support V1[#](#support-v1)
